...
tinyNeoPixelModded::tinyNeoPixelModded(uint16_t n, uint8_t p, neoPixelType t) :
...
tinyNeoPixelModded::tinyNeoPixelModded() :
...
void tinyNeoPixelModded::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
  begun = true;
}

void tinyNeoPixelModded::updateLatch(uint16_t us = 50) {
...
void tinyNeoPixelModded::updateLength(uint16_t n) {
  if (pixels) {
...
void tinyNeoPixelModded::updateType(neoPixelType t) {
...
void tinyNeoPixelModded::show(void) {
  show(0);
}
void tinyNeoPixelModded::show(uint16_t leds) {
...
#else
  void tinyNeoPixelModded::show(uint8_t number_leds) {
...
void tinyNeoPixelModded::setPin(uint8_t p) {
...
void tinyNeoPixelModded::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
  if (n < numLEDs) {
...
void tinyNeoPixelModded::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  if (n < numLEDs) {
    if (brightness) { // See notes in setBrightness()
      r = (r * brightness) >> 8;
      g = (g * brightness) >> 8;
...
void tinyNeoPixelModded::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
*/
void tinyNeoPixelModded::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t tinyNeoPixelModded::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
uint32_t tinyNeoPixelModded::getPixelColor(uint16_t n) const {
  if (n >= numLEDs) {
...
uint8_t *tinyNeoPixelModded::getPixels(void) const {
  return pixels;
}

uint16_t tinyNeoPixelModded::numPixels(void) const {
  return numLEDs;
}
...
void tinyNeoPixelModded::setBrightness(uint8_t b) {
...
uint8_t tinyNeoPixelModded::getBrightness(void) const {
  return brightness - 1;
}

void tinyNeoPixelModded::clear() {
  memset(pixels, 0, numBytes);
}

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t tinyNeoPixelModded::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
